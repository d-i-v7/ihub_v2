...
<div class="modal-body">
    <?php
    $val_id=$_GET['val_id'];
    $_SESSION['imp_id']=$val_id;
$select_val=mysqli_query($conn,"SELECT * FROM impact WHERE id='$val_id'");

if(mysqli_num_rows($select_val)>0)
{
    while($val_row=mysqli_fetch_assoc($select_val))
    {
        $val_topic=$val_row['name'];
$icon=$val_row['icon'];
$val_text=$val_row['number'];
...
?>
   <script>
    document.addEventListener("DOMContentLoaded", function () {
        ...
        <form class="form-group" action="impact_update.php"  method="post" enctype="multipart/form-data">
        <label for="Main_Topic">Impact Icon</label>
        <input id="iconInput" value="<?php echo $icon; ?>" class="from-control" name="icon" type="text">
        <label for="Main_Topic">Impact Name</label>
          <input value="<?php echo $val_topic ; ?>" class="from-control" name="val_topic"  type="text">
          <label for="Main_Topic">Impact Number</label>
          <input value="<?php echo $val_text ; ?>" class="from-control" name="val_text"  type="text">

      
      </div>
      <div class="modal-footer">
        <a  class="btn btn-secondary" href="impact.php" >Close</a>
