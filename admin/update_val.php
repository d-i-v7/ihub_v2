...
$select_val=mysqli_query($conn,"SELECT * FROM our_val WHERE id='$val_id'");

if(mysqli_num_rows($select_val)>0)
{
    while($val_row=mysqli_fetch_assoc($select_val))
    {
        $val_topic=$val_row['val_topic'];
$val_text=$val_row['val_text'];


?>
        <form class="form-group" action="val_update.php"  method="post" enctype="multipart/form-data">
        <label for="Main_Topic">Value Topic</label>
          ...
          <textarea  class="form-control" name="val_text" id="" cols="30" rows="10"><?php echo $val_text ; ?></textarea>
      
      </div>
      <div class="modal-footer">
        <a  class="btn btn-secondary" href="our_val.php" >Close</a>
